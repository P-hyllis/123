...
class LuceneSearchInterface(object):
    ...
    def do_retrieval(self, text, num_hits, num_thread=12):
        if isinstance(text, str):
            ...
            # --- Fix for string/int key mismatch ---
            # 确保所有 key 都是字符串
            results_strkey = {str(k): v for k, v in results.items()}

            results_ordered = []
            missing = []
            for tid in pseudo_batch_topic_ids:
                tid = str(tid)
                if tid in results_strkey:
                    results_ordered.append(results_strkey[tid])
                else:
                    missing.append(tid)
                    results_ordered.append([])  # 可以改成 None 或空列表
            if missing:
                print(f"[WARN] Missing retrieval results for topic IDs: {missing}")
...
def main():
    ...
    parser.add_argument("--generation_model", type=str, default="/mnt/nvme0n1/zcl/IR/Think_QE/Qwen2.5-1.5B")
